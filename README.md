...
**对于 Windows 用户：**
1、按下键盘上的 Win + R 键，在弹出的运行窗口中输入 cmd 并按回车，打开命令提示符 (CMD)。

2、(可选) 使用 cd 命令切换到你想存放项目的文件夹，例如：cd C:\Users\YourName\Desktop

3、在 CMD 中复制并粘贴以下命令来创建虚拟环境：
```bash
# 1. 创建名为 venv 的虚拟环境
python -m venv venv
...
**使用示例：**
> **You:** "帮我计算一下2023年苹果的earnings response cofficient。"
> 
...
4. **结果稳定性**：由于网络波动或模型性能限制，可能会有概率报错，表现为第三步无任何结果输出，此时退出重新提问或稍等提问即可。
